...
export default function TournamentPage() {
  ...
  const user = { id: 2, name: "Diana", isAdmin: true };
  ...
  const handleCreateTournament = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/tournament/create", {
        ...
        body: JSON.stringify({ name: `Tournament ${maxPlayers} Players`, maxPlayers }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const canRegister = tournament?.state === "waiting" && !isRegistered && (tournament.registeredPlayers?.length || 0) < (tournament.maxPlayers || maxPlayers);
      ...
      {tournament && (
        <div className="flex flex-col gap-4 items-center max-w-md">
          <div className="bg-gray-800 p-6 rounded-lg w-full">
            <p className="text-xl mb-2 font-bold">{tournament.name} - ID:  {tournament.id}</p>
            <p className="text-lg">Status: <span className="capitalize font-semibold">{tournament.state}</span></p>
            <p className="text-lg">Registered: {tournament.registeredPlayers?.length || 0}/{tournament.maxPlayers || maxPlayers}</p>
          </div>
          ...
          {user.isAdmin && tournament.state === "waiting" && (tournament.registeredPlayers?.length || 0) === (tournament.maxPlayers || maxPlayers) && (
            <button 
          ...
          {tournament.state !== "waiting" && (
            <div className="bg-green-800 p-4 rounded text-center w-full">
              ...
              {isRegistered ? (
                <p className="text-sm mt-2">You can see your matches in the active tournament page.</p>
              ) : (
