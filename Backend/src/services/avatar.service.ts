...
export const AvatarService = {
  async uploadMyAvatar(input: UploadAvatarInput) {
    ...
    const prev = await AvatarRepo.getSelectedForUser(input.userId);
    ...
    if (prev && !prev.is_default && prev.user_id === input.userId) {
      // prev.file_path is like "uploads/u19/xxxx.png" OR "default_avatars/a1.jpg"
      // we only delete non-default ones; also ensure it's in uploads/
      if (prev.file_path.startsWith("uploads/")) {
        const oldAbs = path.join(process.cwd(), "assets", prev.file_path);
        await fs.unlink(oldAbs).catch(() => {});
      }
    }
    return {
      avatarId,
      avatarUrl: `/static/${relPath}`,
      filePath: relPath,
    };
  },
};
