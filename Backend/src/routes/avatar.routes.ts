...
export async function registerAvatarRoutes(server: FastifyInstance) {
    server.post("/api/me/avatar",
      ...
      if (!file) return reply.code(200).send({ ok:false, error: "Missing file (fiels name avatar)" });
      if (file.fieldname !== "avatar") {
        file.file.resume();
        return reply.code(200).send({ ok:false, error: `Invalid field name "${file.fieldname}" (expected: "avatar")`,});
